...
const IncomeList: React.FC<IncomeListProps> = ({ 
  ...
  hideBalances, onToggleHideBalances, onNavigate, onDeleteIncome,
  showDecimals
}) => {
    ...
    <div className="px-5 pt-12">
      <header className="mb-6 flex justify-between items-end">
        ...
        <div className="flex flex-col gap-2 items-end">
          <button 
            onClick={() => onNavigate('income_categories')}
      ...
      <div className="theme-card rounded-3xl p-6 mb-8 flex justify-between items-center group bg-emerald-50 border-emerald-100">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            <p className="text-[11px] font-bold text-emerald-800 opacity-60 uppercase">Totale Entrate</p>
            <button 
          ...
          <p className="text-3xl font-bold text-emerald-900">
            {hideBalances ? '€ ••••' : `€${formatAmount(filteredIncomes.reduce((s, i) => s + i.amount, 0))}`}
          </p>
        </div>
      </div>

      <div className="space-y-3">
        <h3 className="text-[11px] font-bold opacity-60 uppercase ml-4 mb-2">Entrate Recenti</h3>
        {filteredIncomes.length === 0 ? (
          <div className="text-center py-16 opacity-30">
            <p className="font-semibold italic text-sm">Nessuna entrata</p>
          </div>
        ) : (
          <div className="theme-card rounded-[2rem] overflow-hidden divide-y theme-border">
            {filteredIncomes.map(income => {
              ...
              return (
                <div key={income.id} className="p-4 flex items-center gap-4 active:bg-emerald-50 transition-colors group">
                  <div className="w-11 h-11 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${cat?.color || '#eee'}15` }}>
                    <CategoryIcon iconName={cat?.icon || 'generic'} color={cat?.color || '#10B981'} className="w-5 h-5" />
                  </div>
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-[11px] opacity-60 font-medium">
                          {new Date(income.date).toLocaleDateString('it-IT', { day: 'numeric', month: 'short' })}
                          {acc && ` • ${acc.name}`}
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-bold text-[16px] text-emerald-600">
                          {hideBalances ? '+€ ••' : `+€${formatAmount(income.amount)}`}
                        </span>
                        <button onClick={() => onDeleteIncome(income.id)} className="opacity-0 group-hover:opacity-100 p-1 text-[#D9D1C5] hover:text-rose-400 transition-all">
